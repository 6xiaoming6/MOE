...
class MoE_Wrapper():
    def __init__(self):
        self.device = torch.device('cuda0' if torch.cuda.is_available() is not None else 'cpu')
        ...
        self.region_mask = torch.tensor(pd.read_csv('./model/regions.csv', header=None).values, dtype=torch.float32)
        mask_flat = self.region_mask.reshape(-1).long()
        if mask_flat.min() == 1:
            mask_flat = mask_flat - 1
        projection_mask_matrix = F.one_hot(mask_flat, num_classes=162).float()
        class_counts = projection_mask_matrix.sum(dim=0)
        class_counts = torch.clamp(class_counts, min=1e-6)#防止除零
        self.projection_mask_matrix = projection_mask_matrix / class_counts.unsqueeze(0)

        # 3. 定义 Loss 权重
        self.lambdas = [0.9, 0.1, 0.01] # MSE, Routing, Diversity
        
        self.data = None

        self.logger = self.setup_logger('./experiments/moe_models')

    def setup_logger(self, log_dir='./experiments/moe_models'):
        """设置日志记录器"""
        if not os.path.exists(log_dir):
            os.makedirs(log_dir)
        
        log_file = os.path.join(log_dir, 'moe_training_log.log')
    ...
    def feed_data(self, data):
        """接收来自 JOINT_DIFF 的数据包"""
        self.data = data
        # 数据会被自动解包在 optimize_parameters 里使用

    def mse_loss(self, output, gt32):
        """计算主损失（MSE）"""
        gt_flat = gt32.view(gt32.size(0), -1)  #(batch_size, 32*32)
        gt = torch.matmul(gt_flat, self.projection_mask_matrix.to(self.device))  #(batch_size, 162)]

        mse_loss = F.mse_loss(output, gt)
        mae = F.l1_loss(output, gt)
        rmse = torch.sqrt(mse_loss + 1e-8)
        self.logger.info(f'MoE MSE: {mse_loss.item():.5f}, MoE RMSE: {rmse.item():.5f}, MoE MAE: {mae.item():.5f}')

        return mse_loss

    def netG_forward(self):
        """执行前向传播"""
        # 从 self.data 中取出 JOINT_DIFF 准备好的数据
        x_8 = self.data['moe_input_8']
        x_16 = self.data['moe_input_16'] 
        x_32 = self.data['moe_input_32']
        # print("moe x8 shape:", x_8.shape)
        # print("moe x16 shape:", x_16.shape)
        # print("moe x32 shape:", x_32.shape)

        return self.netG(x_8, x_16, x_32)

    def optimize_parameters(self):
        """单步优化（如果单独训练此模型时用，联合训练时由 JOINT_DIFF 统一调用 step）"""
        self.optG.zero_grad()
        # 联合训练时通常不在这里 step，而是在外部统一 step
        pass

    def save_network(self, current_epoch, root_dir):
        """保存网络权重"""
        current_time = datetime.now().strftime('%Y%m%d_%H%M%S')
        save_filename = f'epoch_{current_epoch}_moe_{current_time}.pth'
...
if __name__ == "__main__":
    ...
    opt['gpu_ids'] = {0}
    net = MoE_Wrapper(opt)
    net.save_network(1, '../experiments/moe_models')